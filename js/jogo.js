...
// Jogador 2: protagonista2HTML aponta para a DIV container (que se move),
// protagonista2 (declarado mais abaixo) aponta para a TAG <img> (que muda o sprite)
...
// ==========================================
// CLASSE DE PROPORÇÃO POR PERSONAGEM (CSS)
// ==========================================
// Cada personagem tem sua própria proporção definida em jogo.css através
// das classes .jack / .morgana / .ruby / .miguel. Essa classe é colada no
// CONTAINER do jogador (protagonistaHTML OU protagonista2HTML) — não no
// "slot" de Jogador 1 ou 2 — para que a proporção siga o personagem
// escolhido, seja ele Jogador 1 ou Jogador 2.
const CLASSES_PERSONAGEM = ["jack", "morgana", "ruby", "miguel"];

function aplicarClassePersonagem(containerEl, nomePersonagem) {
    if (!containerEl) return;
    containerEl.classList.remove(...CLASSES_PERSONAGEM);
    const nome = CLASSES_PERSONAGEM.includes(nomePersonagem) ? nomePersonagem : "morgana";
    containerEl.classList.add(nome);
}
...
// Agora o P2 puxa os sprites do dicionário principal e completão!
const nomeSeguroP2 = resolverPersonagemSeguro(personagemSelecionadoP2);
const spritesJogador2 = dadosPersonagens[nomeSeguroP2].sprites;

// ==========================================
// COLA A CLASSE DE PROPORÇÃO (.jack/.morgana/.ruby/.miguel) NO CONTAINER
// CERTO — a do Jogador 1 recebe a classe do personagem do Jogador 1, a do
// Jogador 2 recebe a classe do personagem do Jogador 2. É isso que faz a
// proporção seguir o personagem, e não o "slot" de jogador 1/2.
// ==========================================
aplicarClassePersonagem(protagonistaHTML, resolverPersonagemSeguro(personagemSelecionado));
aplicarClassePersonagem(protagonista2HTML, resolverPersonagemSeguro(personagemSelecionadoP2));
...
function loopDoJogo(tempoAtual) {
    ...
    if (estaAtirando) {
        ...
        // Mantém agachada proporcionalmente mais baixa
        protagonistaHTML.style.width = "150px";
        protagonistaHTML.style.height = "120px"; /* Ajustado para acompanhar a nova escala */
    }
    else if (pulando) {
        timerAnimacao++;
        if (timerAnimacao >= 6) {
    ...
    if (modoDoisJogadores) {
        ...
        if (estaAtirando2) {
            ...
            if (estaAtirando2) {
                ...
                protagonista2HTML.style.width = "130px";
            ...
            // Mantém a mesma largura do Jogador 1 e só reduz a altura, para não espremer o boneco
            protagonista2HTML.style.width = "150px";
            protagonista2HTML.style.height = "120px";
        }
        else if (pulando2) {
            timerAnimacao2++;
            if (timerAnimacao2 >= 6) {
                timerAnimacao2 = 0;
                framePulo2++;
                if (framePulo2 >= spritesJogador2.pulando.length) {
                    framePulo2 = spritesJogador2.pulando.length - 1;
                }
            }
            protagonista2.src = spritesJogador2.pulando[framePulo2];
            protagonista2HTML.style.width = "150px";
            protagonista2HTML.style.height = "170px";
        }
        ...
        protagonista2HTML.style.left = `${protaX2}px`;   // Corrigido para protagonista2HTML
        protagonista2HTML.style.bottom = `${protaY2}px`; // Corrigido para protagonista2HTML
    }
